...
the voltage applied to the internal circuit. A common modern commercial
application for a pair of mirror galvanometers to be used with a laser is in
animated laser light displays. Given this commercial use, mirror galvanometers
can be sourced for an economic cost.

benefits of using galvanometers as a means of creating a scanning LDV is that
they would address the problem of scaling. In order to scan a larger are the
laser can simply be moved further away from the sound board. There is no
mechanical travel of the laser which would also reduce scanning time. Such a
system was used in the thesis by Rau \cite{RAU_THESIS} to successful measure the
sound board of a guitar sound board. One limitation of digitally controlling a
galvanometer is the bit-depth of the digital to analog converter that is used.
In the thesis by Rau \cite{RAU_THESIS} a 12-bit DAC was used, which provides a
4096 x 4096 grid. A grid of that size would be fine for the size of a guitar or
violin top plate, but in order to accommodate larger soundboards it would be
beneficial to select a DAC of a higher bit-depth.
...
The paper by Le Moyne \cite{MOYNE} provides an example of how laser vibrometry
is used as a tool in musical instrument restoration. use-case, The project would
aim to provide a tool for just such a use-case where an estimation of the
material properties can be used to cross-reference with material properties
found in literature.
...
Element 14 were used as a source for guide prices:
  ...
  \item
    ...
    \tightlist
    \item
        An assembly of multiple stepper motors cascade onto two sets of rails,
        one for movement in 2 dimensions. These rigs can be purchased as a kit
        or assembled from separate parts if a suitable size is not available.
        This assembly can be seen in Figure \ref{fig:ldv-linear-scanner}
